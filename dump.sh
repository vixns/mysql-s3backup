...
if [ "$CREATE_ONE_FILE_PER_DB" = "true" ]; then
  ...
  mysql --skip-column-names -h ${MYSQL_HOST} -u ${MYSQL_USER} -p${MYSQL_PASS} -e 'show databases' \
    ...
    | gzip -9 -c > /backups/${dbname}-${filename}.sql.gz; done \
    || s3cmd --no-progress --access_key=${AWS_ACCESS_KEY} --secret_key=${AWS_SECRET_KEY} put -f /backups/${filename} s3://${S3_BUCKET}/${S3_PATH}dumps/
else
